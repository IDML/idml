  ...
  override def providedFunctions(): List[IdmlFunctionMetadata]          =
    List(
      IdmlFunctionMetadata(
        ...
        "applies a block with the scope of the current expression"),
      IdmlFunctionMetadata(
        "applyArray",
        List("block" -> "block to apply"),
      ...
      IdmlFunctionMetadata(
        "append",
        List("item" -> "item to append"),
        "appends the item to this list"),
      IdmlFunctionMetadata(
        "prepend",
        List("item" -> "item to append"),
        "prepends the item to this list"),
      ...
      IdmlFunctionMetadata(
        "unique",
        List.empty,
        "returns all the unique items in this list, in the order they are encountered"),
      IdmlFunctionMetadata(
        "sort",
      ...
      IdmlFunctionMetadata(
        "languageName",
        List.empty,
        "look up the name of this ISO language code"),
      IdmlFunctionMetadata(
        "languageName",
        List("targetLocale" -> "language code to look up the name in"),
        "look up the name of this ISO language code"
      )
    )
}
