...
class RandomModuleJson(json: IdmlJson) {
  private def seed(p: IdmlValue): Option[MersenneTwister32] = p match {
  ...
  private def random(cursor: IdmlValue): IdmlValue = seed(cursor) match {
    case Some(r) => IdmlValue(r.nextLong())
    case None    => InvalidCaller
  }

  def random2Function(a1: Pipeline, a2: Pipeline): IdmlFunction2 = new IdmlFunction2 {
    override def apply(cursor: IdmlValue, min: IdmlValue, max: IdmlValue): IdmlValue = random(cursor, min, max)
