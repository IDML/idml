...
/** Allows the invocation of IdmlValue methods. Acts as a gateway for calls like 'x.int()' in the mapping language
  *
  ...
  */
case class IdmlValueFunction(method: Method, args: List[Pipeline], isNAry: Boolean = false) extends IdmlFunction {
  ...
  require(classOf[IdmlValue].isAssignableFrom(method.getReturnType), s"method cannot return type ${method.getReturnType}")
  require(
    method.getParameterTypes.forall(p => p.isAssignableFrom(classOf[IdmlValue]) || p == classOf[Seq[IdmlValue]]),
