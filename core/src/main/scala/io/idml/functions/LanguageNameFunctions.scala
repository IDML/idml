...
object LanguageNameFunctions {
  ...
  private def lookupLanguageWithLocale(code: String, targetLocale: Option[String]): IdmlValue = {
    ...
    targetLocale.fold(lang.getDisplayName)(t =>
      lang.getDisplayName(Locale.forLanguageTag(t))) match {
  ...
  case class LanguageNameFunction1(arg: Pipeline) extends IdmlFunction1 {
    override protected def apply(cursor: IdmlValue, targetLocale: IdmlValue): IdmlValue =
      (cursor, targetLocale) match {
