...
class FunctionResolverServiceTest extends AnyFunSuite {
  ...
  test("fails if a function is missing") {
    ...
    intercept[UnknownFunctionException](
      new FunctionResolverService().resolve("missing", twoArgumentExpression))
    intercept[UnknownFunctionException](
      new FunctionResolverService().resolve("slice", threeArgumentExpression))
  }
