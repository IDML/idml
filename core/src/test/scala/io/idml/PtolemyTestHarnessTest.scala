...
class PtolemyTestHarnessTest extends FunSuite {

  /** Counts the number of tests we find */
  class TestCounter extends PtolemyTestHarness {
    var testsRun: Int = 0
    override def executeTest(name: String, test: PtolemyObject): Unit = {
      testsRun += 1
    }
  }

  /** A default implementation of the harness for testing */
  class TestImpl extends PtolemyTestHarness
