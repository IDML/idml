...
object Main extends IOApp {
  ...
  def readFiles[F[_]: Sync](folder: Path): Stream[F, Path] =
    Stream.fromIterator[F](Files.walk(folder).filter(_.toString.endsWith(".md")).iterator().asScala)

  def processFile[F[_]: Sync: Effect: ContextShift](
      in: Path,
      out: Path,
      blocker: Blocker): F[Unit] =
    for {
      contents <-
        fs2.io.file.readAll(in, blocker, 2048).through(fs2.text.utf8Decode[F]).compile.foldMonoid
      ...
      _        <- Stream
                    ...
                    .through(fs2.io.file.writeAll(out, blocker))
                    .compile
                    .drain
    } yield ()

  def processFiles[F[_]: Sync: Effect: ContextShift](
      in: Path,
      out: Path,
      blocker: Blocker): F[Unit] =
    readFiles[F](in)
      .evalMap { file =>
        processFile[F](file, out.resolve(in.relativize(file)), blocker)
      }
      .compile
      .drain
    ...
    CLI.program
      ...
      .map { case (in, out) =>
        Blocker[IO].use { blocker =>
          processFiles[IO](in, out, blocker).as(ExitCode.Success)
        }
      }
      .bisequence
      .map(_.merge)
