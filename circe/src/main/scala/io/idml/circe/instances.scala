...
object instances {
  ...
  lazy val rawIdmlCirceDecoder: Folder[IdmlValue] = new Folder[IdmlValue] {
    ...
    override def onArray(value: Vector[Json]): IdmlValue =
      new IArray(
  ...
  lazy val rawIdmlCirceEncoder: IdmlValue => Json = {
    ...
    case n: IdmlObject  =>
      Json.obj(
        n.fields.toList.filterNot(_._2.isInstanceOf[IdmlNothing]).sortBy(_._1).map { case (k, v) =>
          k -> rawIdmlCirceEncoder(v)
        }: _*
      )
    case n: IdmlArray   =>
  ...
  def decodeObject(value: JsonObject): IdmlObject =
    new IObject(
      value.toIterable.foldLeft(mutable.Map.empty[String, IdmlValue]) { case (acc, (k, v)) =>
        acc += k -> v.foldWith(rawIdmlCirceDecoder)
      }
    )
