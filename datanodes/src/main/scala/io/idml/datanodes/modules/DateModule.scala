...
trait DateModule {
  ...
  /** @see
    ...
    * @return
    *   [[BadDateFormat]] if the date format is invalid, [[CastFailed]] if parsing the date failed
    *   for another reason or a [[io.idml.datanodes.IDate]] representing the results of what was
    *   parsed
    */
  def date(formatStr: IdmlValue): IdmlValue =
    formatStr match {
  ...
  def specificDate(df: DateTimeFormatter): IdmlValue =
    this match {
      case _: IDate | _: IdmlNothing => this
  ...
  def toEpoch(): IdmlValue =
    this match {
      case d: IDate => IInt(d.dateVal.getMillis() / 1000L)
      case _        => CastUnsupported
    }

  def toEpochMillis(): IdmlValue =
    this match {
      case d: IDate => IInt(d.dateVal.getMillis())
      case _        => CastUnsupported
    }

  def timezone(tz: IdmlValue): IdmlValue =
    tz match {
