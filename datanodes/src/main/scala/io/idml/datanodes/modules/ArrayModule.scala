...
trait ArrayModule {
  ...
  def combinations(size: IdmlValue): IdmlValue =
    (this, size) match {
      case (a: IdmlArray, s: IdmlInt) =>
        IArray(a.items.combinations(s.value.toInt).map(xs => IArray(xs)).toBuffer[IdmlValue])
  ...
  def min(): IdmlValue =
    this match {
      case a: IdmlArray =>
        NonEmptyList.fromList(a.items.sorted.toList).map(_.toList.min).getOrElse(NoFields)
      case _            => InvalidCaller
    }
  ...
  def max(): IdmlValue =
    this match {
      case a: IdmlArray =>
        NonEmptyList.fromList(a.items.sorted.toList).map(_.toList.max).getOrElse(NoFields)
      case _            => InvalidCaller
    }
  ...
  def flatten(depth: IdmlValue): IdmlValue =
    (this, depth) match {
  ...
  def zip(other: IdmlValue): IdmlValue =
    (this, other) match {
  ...
  def enumerate(): IdmlValue =
    this match {
      case (a: IdmlArray) =>
        IArray(
          a.items.zipWithIndex
            .map { case (item, idx) => IArray(IdmlValue(idx), item) }
            .toBuffer[IdmlValue])
      case _              =>
        InvalidCaller
    }

  def union(other: IdmlValue): IdmlValue =
    (this, other) match {
  ...
  def intersect(other: IdmlValue): IdmlValue =
    (this, other) match {
  ...
  def diff(other: IdmlValue): IdmlValue =
    (this, other) match {
      ...
      case ar: IdmlArray =>
        IArray(ar.items.filter(i => !i.isNothingValue)) match {
          ...
          case a: IdmlArray if a.items.forall(_.isObject.value) =>
            a.items.asInstanceOf[mutable.Buffer[IdmlObject]].foldLeft(IObject()) { case (o, i) =>
              o.deepMerge(i)
            }
