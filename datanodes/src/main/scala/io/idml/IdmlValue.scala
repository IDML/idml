...
object IdmlValue {
  ...
  private def sortingClass(p: IdmlValue): Int =
    p match {
      case IdmlNull       => 1
      ...
      (sortingClass(x) - sortingClass(y)) match {
        ...
        case i if i == 0 =>
          (x, y) match {
            ...
            case (x: IdmlInt, y: IdmlDouble)    =>
              Ordering[IdmlDouble].compare(new IDouble(x.value.doubleValue()), y)
            case (x: IdmlDouble, y: IdmlInt)    =>
              Ordering[IdmlDouble].compare(x, new IDouble(y.value.doubleValue()))
            case _                              => 0
          }
      }
