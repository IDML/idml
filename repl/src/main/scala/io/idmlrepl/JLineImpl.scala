...
class JLineImpl[F[_]: Effect](data: Ref[F, IdmlObject], term: Option[Terminal]) extends JLine[F] {

  def passthroughParsedLine(s: String, c: Int): ParsedLine =
    new ParsedLine {
  ...
  private val t            = term.getOrElse(TerminalBuilder.builder().system(true).build())
  ...
  private val functions    =
    (StaticFunctionResolverService.defaults(IdmlCirce).asScala ++ List(
      ...
      new HashingFunctionResolver)).toList
      .flatMap { f =>
  ...
  private val c            = new Completer {
    def complete(reader: LineReader, line: ParsedLine, candidates: util.List[Candidate]): Unit = {
      ...
      val suggestions                  =
        Try { AutoComplete.complete(idml)(d, partialLine, partialCursor) }.getOrElse(Set.empty)
  ...
  private val jsonLr       = LineReaderBuilder
    ...
    .variable(
      ...
      Paths.get(System.getProperty("user.home"), ".idml_history_json"))
    .variable(LineReader.SECONDARY_PROMPT_PATTERN, "..... ")
    .history(h2)
    .parser(jsonParser)
    .appName("idml repl")
  ...
  override def printAbove(s: String): F[Unit] =
    Sync[F].delay {
      lr.printAbove(s)
    }

  def read[T](r: F[Either[Throwable, String]])(
      parser: String => Either[Throwable, T]): F[ReadLine[T]] =
    EitherT(r)
      .subflatMap { s =>
  ...
  override val compileJson: String => Either[Throwable, IdmlObject] =
    IdmlCirce.parseObjectEither(_).leftWiden[Throwable]

  override val compileIdml: String => Either[Throwable, Mapping] = (s: String) =>
    Try { idml.compile(s) }.toEither
