...
object IdmlTools {
  ...
  val functionResolver: Opts[FunctionResolverService] = (dynamic, plugins).mapN { (d, pf) =>
    val baseFunctionResolver = if (d) { new FunctionResolverService }
    else {
  ...
  def server(implicit c: ConcurrentEffect[IO], t: Timer[IO]) =
    Command(
      ...
      (bindAll, functionResolver).mapN { case (b, fr) =>
        BlazeBuilder[IO]
  ...
  def apply(implicit cs: ContextShift[IO]) =
    Command(
      ...
      val unmapped  = Opts
        .flag("unmapped", "This probably doesn't do what you think it does", short = "u")
        .orFalse
      ...
      val traceFile = Opts.option[String]("trace", "File to trace into", "t").orNone.mapValidated {
        case Some(f) =>
          Validated
            .fromEither(
              Either
                .catchNonFatal(Some(new File(f)))
                .leftMap(e => s"Invalid file: ${e.getLocalizedMessage}"))
            .toValidatedNel
      ...
      (pretty, unmapped, strict, file, functionResolver, traceFile)
        .mapN { (p, u, s, f, fr, t) =>
          ...
          val analysisModule =
            if (config.traceFile.isDefined) Some(new Annotator(jsonModule)) else None
          ...
          Blocker[IO].use { blocker =>
            {
              for {
                _        <- EitherT
                              .cond[IO](
                                !(config.traceFile.isDefined && config.files.length != 1),
                                (),
                                "When tracing please supply one mapping to run"
                              )
                ...
                compiled <- found.zip(strings).traverse { case (file, s) =>
                              EitherT(IO {
                                Either
