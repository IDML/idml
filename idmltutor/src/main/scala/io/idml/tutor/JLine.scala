...
class JLine[F[_]](name: String, t: Terminal)(implicit F: Sync[F]) {
  ...
  val reader = LineReaderBuilder.builder().terminal(t).appName(name)
    .variable(LineReader.HISTORY_FILE, "~/.idmltutor_history")
    .history(history)
    .build()
  val multiReader = LineReaderBuilder.builder().terminal(t).appName(name)
...
object JLine {
  def apply[F[_]](name: String)(implicit F: Sync[F]): F[JLine[F]] = F.delay {
    new JLine[F](name, TerminalBuilder.builder()
        .jansi(true)
        .build()
    )
  }
