  ...
  def apply[F[_]](tutorial: TutorialAlg[F])(implicit F: Sync[F]) =
    for {
      ...
      _ <- tutorial.content(
         ...
         |  ${grey("foo.bar = a.b.c")}
         |
         |This would set the ${grey("bar")} field inside ${grey("foo")} to the value of the ${grey("c")} field inside ${grey("b")} inside ${grey(
                 "a")}
         |
      ...
      _ <- tutorial.content(
         ...
         |${grey("""result = match target
           ...
           """)}
         |
         |You start off by using the ${grey("match")} keyword to give an expression you're matching over the result of, in this case ${grey(
                 "target")}
         |
