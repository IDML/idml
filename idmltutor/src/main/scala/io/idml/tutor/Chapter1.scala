    ...
    for {
      ...
      _ <-
        tutorial.content(
         ...
         |  ${grey("foo.bar = a.b.c")}
         |
         |This would set the ${grey("bar")} field inside ${grey("foo")} to the value of the ${grey(
              "c")} field inside ${grey(
              "b"
            )} inside ${grey("a")}
         |
         ...
         |
         |If the ${grey("foo")} object didn't exist it would create it when assigning the ${grey(
              "bar")} value inside it, but this would only occur if the right side evaluates to a value.
       """.stripMargin
        )
      _ <- tutorial.pause
      ...
      _ <- tutorial.content(
             s"""
        |Functions are one of the most important aspects of IDML, they're always called as a ${underlined(
                 "suffix")} to an expression, and always have rounded braces on the end.
        |
        |Example:
      ...
      _ <- tutorial.content(s"""
         |IDML provides some operators, which are mostly used for mathematical operations, they are called in an ${underlined(
                                "infix")} style like so:
         |  ${grey("result = a + b")}
       """.stripMargin)
      _ <- tutorial.pause
      ...
      _ <-
        ...
        |  ${grey("foo = (a | b)")}
        |
        |This would set ${grey("foo")} to the value of ${grey(
              "a")} if it's defined, otherwise ${grey("b")} if it's defined, or not set it at all.
      """.stripMargin
        )
      _ <- tutorial.pause
      _ <- tutorial.content(
         ...
         |
         |  ${grey("""cat.name == "martin"""")}       ${green(
                 "# check if the cat's name is martin")}
         |  ${grey("""cat.name != "martin"""")}       ${green(
                 "# check if the cat's name is not martin")}
         |  ${grey("""cat.name in ["bob", "tom"]""")} ${green(
                 "# check if the cat's name is in our list of cat names")}
         |  ${grey("""cat.age > 5""")}                ${green(
                 "# check if the cat's age is above 5")}
         |  ${grey("""cat.name exists""")}            ${green("# check if the cat has a name")}
         |
         |These can be combined or modified using the ${grey("not")} prefix operator, or the ${grey(
                 "and")} and ${grey("or")} infix operators.
         |
      ...
      _ <-
        tutorial.content(
         ...
         |
         |  ${grey("if foo then bar")} ${green(
              "# evaluates to the result of the expression bar if the predicate foo evaluates to true")}
         |  ${grey("if foo then bar else baz")} ${green(
              "# the same as above, but we fall through to the expression baz if it's false")}
         |
         |a practical example might be:
         |
         |  ${grey("""content = if type == "tweet" then tweet.content""")} ${green(
              "# set the content to tweet.content if type is tweet")}
       """.stripMargin)
      _ <- tutorial.pause
      _ <- tutorial.section("Exercise 4: Branching")
      _ <- tutorial.exerciseMulti(
             s"""
          |Please map the input objects to the output objects, you'll want to change behaviour based on the value in ${grey(
                 "target")}
       """.stripMargin,
             List(
      ...
      _ <-
        tutorial.content(
         ...
         |${grey("""result = match target
              || this == "a" => a
              || this == "b" => b
           """)}
         |
         |You start off by using the ${grey(
              "match")} keyword to give an expression you're matching over the result of, in this case ${grey(
              "target"
            )}
         |
         |You then give a set of cases on new lines, each starting with a ${grey(
              "|")}, then a predicate, then a ${grey("=>")} and the expression to evaluate if the predicate matches.
         |
