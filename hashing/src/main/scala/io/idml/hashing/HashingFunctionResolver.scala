...
import io.idml.datanodes.{IInt, IString}
...
object HashingFunctions {
  ...
  val hashes: Map[String, IdmlFunction0] = Map(
    ...
    hashFunction("md5")((s: String) => IString(Hashing.md5().hashBytes(s.getBytes).toString)),
    hashFunction("crc232")((s: String) => IString(Hashing.crc32().hashBytes(s.getBytes).toString))
  )
}
