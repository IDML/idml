...
object PValueComparison {
  ...
  def assertEqual(left: PtolemyValue, right: PtolemyValue, path: List[String] = Nil): Unit = {
    (left, right) match {
      case (l: PtolemyObject, r: PtolemyObject) =>
      ...
      case (l: PtolemyArray, r: PtolemyArray) =>
        l.items.zip(r.items).zipWithIndex.foreach {
          case ((ll: PtolemyValue, rr: PtolemyValue), i: Int) =>
