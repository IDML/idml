  ...
  "JsonAstGenerator" when {
    "fed a simple single level schema with 1 item" should {
      "correctly represent the IDML as JSON" in {
        parser.parse(funcs, "foo : int()").toJson must equal(
          """[{"operation":"reassignment","destination":"foo","type":"int"}]""")
        parser.parse(funcs, "bar : string()").toJson must equal(
          """[{"operation":"reassignment","destination":"bar","type":"string"}]""")
        parser.parse(funcs, "baz : bool()").toJson must equal(
          """[{"operation":"reassignment","destination":"baz","type":"bool"}]""")
        parser.parse(funcs, "MixedCase : date()").toJson must equal(
          """[{"operation":"reassignment","destination":"MixedCase","type":"date"}]"""
        )
        parser.parse(funcs, "earl: url()").toJson must equal(
          """[{"operation":"reassignment","destination":"earl","type":"url"}]""")
      }
    }
    ...
    "fed a simple multi-level schema with 1 item" should {
      "correctly represent the IDML as JSON" in {
        parser.parse(funcs, "foo.baz : int()").toJson must equal(
          """[{"operation":"reassignment","destination":"foo.baz","type":"int"}]""")
